...
import numpy as np
import torch
from monai.networks.layers import calculate_out_shape
...
def log_normal(x, mu=None, v=None, broadcast_size=False):
    ...
    return -0.5 * (torch.log(2 * torch.tensor(math.pi)) + v.log() + (x - mu).pow(2).div(v))
...
class cleanIVAE(nn.Module):
    ...
    def elbo(self, x, u, N, a=1., b=1., c=1., d=1.):
        ...
        M, d_latent = z.size()  # (bs, dim)
        logpx = log_normal(x, f, self.decoder_var.to(x.device)).sum(dim=-1)  # (bs)
        logqs_cux = log_normal(z, g, v).sum(dim=-1)  # (bs)
        logps_cu = log_normal(z, None, l).sum(dim=-1)  # (bs)

        # no view for v to account for case where it is a float. It works for general case because mu shape is (1, M, d)
        logqs_tmp = log_normal(z.view(M, 1, d_latent), g.view(1, M, d_latent), v.view(1, M, d_latent))  # (bs, bs, dim)
        logqs = torch.logsumexp(logqs_tmp.sum(dim=-1), dim=1, keepdim=False) - np.log(M * N)  # (bs)
        logqs_i = (torch.logsumexp(logqs_tmp, dim=1, keepdim=False) - np.log(M * N)).sum(dim=-1)  # (bs)
...
class iVAEforCBCT(nn.Module):
    def __init__(self, in_shape, latent_dim, hidden_dim=256):
        super().__init__()
        ...
        # Calculate size after conv encoder
        self.final_size = np.asarray(self.vae.in_shape, dtype=int)

        for s in self.vae.strides:
            self.final_size = calculate_out_shape(self.final_size, self.vae.kernel_size, s, 1)

        self.encoded_latent_size = np.multiply.reduce(self.final_size) * self.vae.channels[-1]

        # prior params
        self.prior_mean = torch.zeros(1)
        self.logl = MLP(1, latent_dim, self.hidden_dim, 3, activation='xtanh', slope=0.1)

        # decoder params
        self.pre_f = MLP(latent_dim, self.encoded_latent_size, self.hidden_dim, 3, activation='xtanh', slope=0.1)
        self.f = copy.deepcopy(self.vae.decode)
        self.decoder_var = .1 * torch.ones(1)

        # encoder params
        self.g = nn.Sequential(
            copy.deepcopy(self.vae.encode),
            nn.Flatten(1),
            MLP(self.encoded_latent_size, latent_dim, self.hidden_dim, 3, activation='xtanh', slope=0.1)
        )
        self.logv = nn.Sequential(
            copy.deepcopy(self.vae.encode),
            nn.Flatten(1),
            MLP(self.encoded_latent_size, latent_dim, self.hidden_dim, 3, activation='xtanh', slope=0.1)
        )
    ...
    def encoder(self, x, u):
        u = u.view(u.shape[0], 1, 1, 1, 1).expand_as(x)
        xu = torch.cat((x, u), 1)

        g = self.g(xu)
        logv = self.logv(xu)

        return g.flatten(1), logv.exp().flatten(1)

    def decoder(self, s):
        s = self.pre_f(s)
        s = s.view(s.shape[0], self.vae.channels[-1], *self.final_size)
        return self.f(s)
    ...
    def elbo(self, x, u, N, a=1., b=1., c=1., d=1.):
        ...
        # log prob of obs under normal dist with mean recon and variance 0.1
        logpx = log_normal(x, f, self.decoder_var.to(x.device)).sum(dim=[1, 2, 3, 4])

        # log prob of mean of the prior under normal dist with mean of obs sample and log variance of obs sample
        logqs_cux = log_normal(z, g, v).sum(dim=-1)
...
if __name__ == "__main__":
    x = torch.rand((2, 1, 256, 256, 128))
    u = torch.rand((2, 1))
    model = iVAEforCBCT(in_shape=(2, 256, 256, 128), latent_dim=256)
    y_pred = model(x, u)
    pass
